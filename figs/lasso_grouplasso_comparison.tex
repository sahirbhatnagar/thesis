\begin{figure}
		...
		\FigureCaptionOpt{Visualizing the key difference between the lasso and group lasso penalties}{The selected components from Model~\eqref{eq:credit_card} for (a) the lasso and (b) the group lasso. The origin of the $xyz$ axis is zero which means both the linear and quadratic terms are 0. In this toy example, the lasso selects only the quadratic term for age while the group lasso selects both linear and quadratic terms.}\label{fig:lasso_grouplasso_comparison}
\end{figure}
