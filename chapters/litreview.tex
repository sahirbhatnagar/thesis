...
Linear mixed-effects models (LMMs)~\citep{laird1982random} are a class of statistical models used to account for correlations induced by a natural grouping of the observations such as students in schools or repeated measurements on the same individual. Ignoring this grouping structure can lead to incorrect inference due to biased standard errors of parameters, and can also violate the normality of residuals assumption. By introducing a subject specific random effect, LMMs can simultaneously model the mean and covariance structure. In this section we briefly introduce LMMs as a preface to Chapter 4 of the thesis which is about penalized LMMs.
