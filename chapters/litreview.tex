...
Using~\eqref{eq:kkt_glasso} we can solve for the smallest value of $\lambda$ such that the entire vector $\left\lbrace \hat{\bbeta}^{(k)}\right\rbrace_{k=1}^{K}$ is 0. This is given by
 \begin{equation}
 \lambda_{max} = \max_k \left\lbrace \abs{ \frac{1}{v_j} \sum_{i=1}^{n}w_i X_{ij}y_i }\right\rbrace , \quad j=1, \ldots, p
 \end{equation}


In Figure~\ref{fig:gglasso_maj}
