...
\begin{equation}
Q_{\lambda}(x) = \argmin_{x}\; f(x) + \lambda P(x) \label{eq:tseng}
\end{equation}
At each iteration, the algorithm approximates $f(x)$ in~\eqref{eq:tseng} by a strictly convex quadratic function and then applies block coordinate decent to generate a decent direction followed by an inexact line search along this direction~\citep{tseng2009coordinate}. 
For continuously differentiable $f(\cdot)$ and convex and block-separable $P(\cdot)$ \mbox{(i.e. $P(\bbeta) = \sum_i P_i (\beta_i)$)},~\cite{tseng2009coordinate} show that the solution generated by the CGD method is a stationary point of~\eqref{eq:tseng} if the coordinates are updated in a Gauss-Seidel manner i.e. $Q_{\lambda}(x)$ is minimized with respect to one parameter while holding all others fixed. 
...
Indeed, the CGD algorithm has been successfully applied in fixed effects models (e.g.~\cite{meier2008group},~\cite{friedman2010regularization}) and~\cite{schelldorfer2011estimation} for mixed models with an $\ell_1$ penalty. Following~\cite{tseng2009coordinate}, the general purpose CGD algorithm is given by Algorithm~\ref{alg:cgd}. 
...
Below we detail the specifics of Algorithm~\ref{alg:cgd} for the $\ell_1$ penalty with objective function given by
\begin{equation}
Q_{\lambda}(\bTheta) = f(\bTheta) + \lambda \norm{\bbeta}
\end{equation}
