...
\end{equation}
where $\gamma \in \mathbb{R}^p$ is the subgradient of the function $f(x) = \abs{x}$ evaluated at $x = \hat{\beta}$. From~\eqref{eq:kktsolved}, we can solve for the smallest value of $\lambda$ such that the entire vector ($\hat{\beta}_1, \ldots, \hat{\beta}_p$) is 0. This is given by
\begin{equation}
